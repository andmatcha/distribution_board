...
# macOSが生成する不要ファイルを削除する関数
remove_ignored_files() {
    local dir="$1"

    if [ -f "$dir/.DS_Store" ]; then
        log_info "不要なファイルを削除: $dir/.DS_Store"
        rm -f "$dir/.DS_Store"
    fi
}
...
move_directory() {
    ...
    # 元のディレクトリが空の場合はスキップ
    if is_effectively_empty_dir "$src"; then
    ...
    # 元のディレクトリが空になったか確認
    if [ -d "$src" ] && is_effectively_empty_dir "$src"; then
...
cleanup_empty_parent_dirs() {
    ...
    while [ -d "$dir" ] && [ "$dir" != "." ] && [ "$dir" != "/" ] && is_effectively_empty_dir "$dir"; do
...
main() {
    ...
    # 3. Core/Startup -> src
    move_directory "./Core/Startup" "./src"

    # 4. Drivers -> lib/Drivers
