...
- `board4`: エンコーダ位置を CAN ID `0x304` で送信しつつ、CAN 受信でサーボ 1 系統を制御します。
- `board4u`: STM32F446RETx で USB Host MSC の外部ストレージから座標テキストを読み、CAN2 へ送信します。あわせて CAN2 受信で DC モータ 2 系統を制御します。
...
board4u は CAN2 filter を `0x208` のみに設定し、メインループで FIFO0 をポーリングします。同じ `0x208` フレーム内で、座標要求と DC モータ指令を同時に扱えます。

| CAN ID | DLC | payload | 処理 |
| --- | --- | --- | --- |
| `0x208` | `>=3` | `data[2]` | `1` なら USB ストレージから次の座標を読み、`0x209` で応答します。 |
...
board4 は board2/3 と同じエンコーダ送信に加えて、CAN 受信でサーボを制御します。
...
- INA219 は `I2C1` の 7-bit address `0x40` で接続し、5 mΩ シャント前提で calibration register を設定します。debug build では 500 ms ごとにサーボ電源の bus voltage、shunt voltage、current、power をログ出力します。シャント抵抗を変える場合は `board4/include/board_config.h` の `BOARD_SERVO_INA219_SHUNT_MILLIOHM` を変更します。

## board4u 実装

board4u は USB Host MSC と FatFs を使い、外部ストレージ内の `.txt` ファイルから緯度経度を読み取ります。CAN2 で標準 ID `0x208`、DLC `>=3`、`data[2] == 1` の要求を受けると、次の有効な座標ファイルを読み、標準 ID `0x209`、DLC `8` で `latitude_e7` と `longitude_e7` を little-endian の `int32_t` として送信します。

ファイル探索は USB ドライブ配下を最大 4 階層まで再帰し、読み取りに成功したファイルの次から巡回します。対応する座標表現は `lat/lon` ラベル付きの数値、または 2 つの座標値の組です。
